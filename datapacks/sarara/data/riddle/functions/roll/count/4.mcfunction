...
#room4上げる
execute if score time roll matches 1 run function riddle:roll/room/summon/4
execute if score time roll matches 1..80 as @e[type=armor_stand,tag=room4_move] at @s run tp @s ~ ~0.04 ~

#room3下げる
execute if score time roll matches 1..80 as @e[type=armor_stand,tag=room3_move] at @s run tp @s ~ ~-0.0425 ~

#入口検知(閉じる時room3)
execute if score time roll matches 1..80 as @a[x=-117,y=51,z=-31,dx=0,dy=2,dz=-2] at @s run tp @s ~1 ~ ~