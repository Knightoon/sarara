...
#room2上げる
execute if score time roll matches 1 run function riddle:roll/room/summon/2
execute if score time roll matches 1..80 as @e[type=armor_stand,tag=room2_move] at @s run tp @s ~ ~0.04 ~

#room1下げる
execute if score time roll matches 1..80 as @e[type=armor_stand,tag=room1_move] at @s run tp @s ~ ~-0.0425 ~

#入口検知(閉じる時room1)
execute if score time roll matches 1..80 as @a[x=-117,y=51,z=-48,dx=0,dy=2,dz=-2] at @s run tp @s ~1 ~ ~
execute if score time roll matches 1..80 as @a[x=-137,y=51,z=-60,dx=0,dy=2,dz=-2] at @s run tp @s ~-1 ~ ~